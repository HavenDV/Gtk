...
$ManifestBaseName = "Gtk.Sdk.Manifest"
...
function Install-GtkWorkload([string]$DotnetVersion, [string]$Source)
{
    $VersionSplitSymbol = '.'
    ...
    $GtkManifestDir = Join-Path -Path $ManifestDir -ChildPath "gtk.sdk.manifest"
...
else
{
    foreach ($DotnetSdk in $InstalledDotnetSdks)
    {
        try {
            ...
            Install-GtkWorkload -DotnetVersion $DotnetSdk -Source $Source
        }
        catch {
