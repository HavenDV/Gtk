...
MANIFEST_NAME="Gtk.Sdk.Manifest"
...
function install_gtkworkload() {
    ...
    mkdir -p $SDK_MANIFESTS_DIR/gtk.sdk.manifest
    cp -f $TMPDIR/unzipped/data/* $SDK_MANIFESTS_DIR/gtk.sdk.manifest/

    if [ ! -f $SDK_MANIFESTS_DIR/gtk.sdk.manifest/WorkloadManifest.json ]; then
    ...
    if [[ "$SOURCE" == "<auto>" ]]; then
      ...
      $DOTNET_INSTALL_DIR/dotnet workload install gtk --skip-manifest-update --source $SOURCE
    fi
...
if [ -z "$INSTALLED_DOTNET_SDKS" ]; then
    ...
    for DOTNET_SDK in $INSTALLED_DOTNET_SDKS; do
        ...
        install_gtkworkload $DOTNET_SDK $SOURCE
    done
fi
